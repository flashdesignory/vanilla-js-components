export class EventsObserver{
  ...
  connect() {
    console.log("connect()");
    this.events.forEach(entry => this.ref.addEventListener(...Object.values(entry)));
  }

  disconnect() {
    console.log("deconnect()");
    this.events.forEach(entry => this.ref.removeEventListener(...Object.values(entry)));
  }
  ...
  handleOnMutation(mutationList) {
    for (const mutation of mutationList) {
      if (mutation.type === 'childList') {
        const refWasAdded = Array.from(mutation.addedNodes).find(node => node === this.ref)
        const refWasRemoved = Array.from(mutation.removedNodes).find(node => node === this.ref)
